...
export const get_messages_customer = createAsyncThunk(
    "chat/get_message_customers",
...
export const seller_send_message_to_customer = createAsyncThunk(
    "chat/seller_send_message_to_customer",
...
export const chatReducer = createSlice({
    ...
    initialState: {
        ...
        messages: [], // message seller & customer
        activeCustomer: [],
        activeSeller: [],
        activeAdmin: "",
        friends: [], //list friend of seller
        seller_admin_message: [], // message admin & seller
        currentSeller: {},
        currentCustomer: {},
        sellers: [], // list seller -> dashboard admin
    },
    reducers: {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(get_messages_customer.fulfilled, (state, { payload }) => {
                state.messages = payload.messages;
                state.currentCustomer = payload.currentCustomer;
            })
            .addCase(
                seller_send_message_to_customer.fulfilled,
                (state, { payload }) => {
                    let tempFriends = state.customers;
                    let index = tempFriends.findIndex(
                        (f) => f.fdId === state.currentCustomer
                    );
                    while (index > 0) {
